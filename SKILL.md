...
7. In reception-history prose, use `reception` for the full historical process, including use, interpretation, approval, resistance, misunderstanding, and delayed recognition. Reserve `acceptance` for demonstrated approval or adoption. Use `received as X` only when evidence identifies an actual receiver or receiver group and supports interpretation X; textual availability alone proves an offered interpretive position, not uptake.
